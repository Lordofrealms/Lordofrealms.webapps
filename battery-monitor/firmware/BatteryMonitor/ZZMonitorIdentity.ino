// Battery Monitor P0-3 authenticated monitoring identity.
//
// A stable random 256-bit Monitoring Identity Key authenticates discovery and
// battery status independently from the user's Device Password. The key is
// exported only through an authenticated P0-2 management session and is wrapped
// with a session-specific AES-256-GCM key before crossing the LAN.
...
void handleMonitoringKeyExport() {
  ...
  if (!monitoringIdentityReady && !loadOrCreateMonitoringIdentity()) {
    server.send(500, "application/json", "{\"error\":\"monitoring identity unavailable\"}");
    return;
  }
  ...
  if (rc == 0) {
    rc = mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT,
                                   sizeof(monitoringIdentityKey),
                                   iv, sizeof(iv),
                                   (const unsigned char*)aad.c_str(), aad.length(),
                                   monitoringIdentityKey, ciphertext,
                                   sizeof(tag), tag);
  }
  ...
  if (rc != 0) {
    ...
    server.send(500, "application/json", "{\"error\":\"monitoring key encryption failed\"}");
    return;
  }
...
void handleAuthenticatedStatus() {
  if (!server.hasArg("nonce")) {
    server.send(400, "application/json", "{\"error\":\"nonce required\"}");
    return;
  }
  String nonce = server.arg("nonce");
  nonce.toLowerCase();
  if (!validMonitorNonce(nonce)) {
    server.send(400, "application/json", "{\"error\":\"invalid nonce\"}");
    return;
  }
  if (!monitoringIdentityReady && !loadOrCreateMonitoringIdentity()) {
    server.send(500, "application/json", "{\"error\":\"monitoring identity unavailable\"}");
  ...
  if (!monitoringHmac("BATMON-STATUS-V1", nonce, payload, mac)) {
    server.send(500, "application/json", "{\"error\":\"status authentication failed\"}");
    return;
  }
...
void registerMonitoringIdentityRoutes() {
  server.on("/api/status-auth", HTTP_GET, handleAuthenticatedStatus);
  server.on("/api/monitor-key", HTTP_POST, handleMonitoringKeyExport);
}
