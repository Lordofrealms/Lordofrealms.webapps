...
public final class PadGradeNativeBridge implements PrecisionLocationClient.Listener, SensorEventListener {
    ...
    @JavascriptInterface public boolean startHeadingUpdates() {
        if (sensorManager == null || rotationSensor == null) return false;
        if (!headingActive) headingActive = sensorManager.registerListener(this, rotationSensor, SensorManager.SENSOR_DELAY_UI);
    ...
    @JavascriptInterface public boolean saveTextFile(String filename, String mimeType, String text) {
    ...
    @JavascriptInterface public String listProjectFiles() {
        ...
        for (DocumentFile file : folder.listFiles()) {
            ...
            if (file.isFile() && name != null && name.toLowerCase().endsWith(".padgrade")) out.put(name);
        }
        return out.toString();
    }
    ...
    public void onProjectFolderSelected(Uri uri) {
        if (uri == null) return;
        activity.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString(PROJECT_FOLDER_URI, uri.toString()).apply();
    ...
    private DocumentFile getProjectFolder() {
        ...
        try {
            ...
            return folder != null && folder.exists() && folder.canRead() && folder.canWrite() ? folder : null;
        } catch (Exception ex) {
            return null;
        }
    }
    ...
    private DocumentFile findProjectFile(String filename) {
        ...
        for (DocumentFile file : folder.listFiles()) if (filename.equals(file.getName())) return file;
        return null;
    }
    ...
    @Override public void onPrecisionLocation(String jsonPayload) {
        ...
        evaluate("window.__padGradeNativeLocation && window.__padGradeNativeLocation(" + JSONObject.quote(jsonPayload) + ");");
    }

    @Override public void onPrecisionError(String message) {
        evaluate("window.__padGradeNativeLocationError && window.__padGradeNativeLocationError(" + JSONObject.quote(message == null ? "Precision Location error" : message) + ");");
    }
    ...
    @Override public void onSensorChanged(SensorEvent event) {
        ...
        float[] base = new float[9]; float[] adjusted = new float[9];
        ...
        int xAxis = SensorManager.AXIS_X, yAxis = SensorManager.AXIS_Y;
        if (rotation == Surface.ROTATION_90) { xAxis = SensorManager.AXIS_Y; yAxis = SensorManager.AXIS_MINUS_X; }
        else if (rotation == Surface.ROTATION_180) { xAxis = SensorManager.AXIS_MINUS_X; yAxis = SensorManager.AXIS_MINUS_Y; }
        else if (rotation == Surface.ROTATION_270) { xAxis = SensorManager.AXIS_MINUS_Y; yAxis = SensorManager.AXIS_X; }
        SensorManager.remapCoordinateSystem(base, xAxis, yAxis, adjusted);
        float[] orientation = new float[3]; SensorManager.getOrientation(adjusted, orientation);
        ...
        if (Double.isFinite(lastLatitude) && Double.isFinite(lastLongitude)) {
            ...
            declination = new GeomagneticField((float) lastLatitude, (float) lastLongitude, (float) lastAltitude, when).getDeclination();
        }
        smoothedHeading = smoothAngle(smoothedHeading, normalize(magneticHeading + declination), 0.22f);
        JSONObject payload = new JSONObject();
        try { payload.put("heading", smoothedHeading); payload.put("accuracy", JSONObject.NULL); payload.put("source", "Android rotation vector"); } catch (Exception ignored) {}
        evaluate("window.__padGradeNativeHeading && window.__padGradeNativeHeading(" + JSONObject.quote(payload.toString()) + ");");
    }
