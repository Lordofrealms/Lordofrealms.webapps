/* Pad Grade v1.3.6 DEV — Blob-bundled parallel coordinator for 99 / 297 / 891.
 *
 * v1.3.5 proved the Blob child-worker transport on Android. v1.3.6 applies the
 * same compute-only band path to every progressive heatmap tier so field timing
 * can be compared directly with the sequential 99/297 timings already captured
 * in prior logs. No sequential reference pass is added here.
 *
 * Bands remain offscreen; only one complete assembled RGBA buffer per tier can
 * leave this coordinator, preserving the existing no-row-painting/flicker gate.
 */
'use strict';
...
const BAND_SOURCE_URL=new URL('heatmap-raster-band-worker-v136.js?v=20260901-1',self.location.href).href;
...
function buildWhole(msg,extra={}){
  ...
  postMessage({
    ...
    colorElapsedMs:colored.colorElapsedMs,setupElapsedMs:+(rasterStarted-state.started).toFixed(1),parallelMetrics:{
      ...
      fallbackReason:extra.fallbackReason||null,childWorkerKind:'blob-bundled-v136',parallelTierPolicy:'99-297-891',
      blobPrepElapsedMs:extra.blobPrepElapsedMs??null,blobSourceBytes:extra.blobSourceBytes??null,
      atomicFinalBuffer:true,partialFramesPublished:0,bandFramesPublished:0,sequentialBenchmarkRun:false
...
async function prepareBundledWorkerBlob(){
  ...
  const [surfaceSource,bandSource]=await Promise.all([fetchAssetText(SURFACE_URL),fetchAssetText(BAND_SOURCE_URL)]);
...
async function startParallel(msg){
  ...
  const coordinator={
    jobId:msg.jobId,tier:+msg.tier||0,started:state.started,workers:[],remaining:desired,cancelled:false,fallingBack:false,
  ...
  try{bundle=await prepareBundledWorkerBlob();}
    ...
    coordinator.fallingBack=true;activeCoordinator=null;stopWorkers(coordinator);
  ...
  coordinator.blobUrl=bundle.blobUrl;coordinator.blobSourceBytes=bundle.blobSourceBytes;coordinator.blobPrepElapsedMs=bundle.blobPrepElapsedMs;
  const createStarted=pgWorkerNow();

  let createError=null;
  for(let i=0;i<desired;i++){
    const band=bandSpec(i,desired,state.ny,state.length),bandState={index:i,stage:'not-created',stageElapsedMs:null};
    coordinator.bandStates[i]=bandState;
    if(!band.rows){coordinator.remaining--;bandState.stage='empty-spec';continue;}
    let w=null;
    try{w=new Worker(coordinator.blobUrl);bandState.stage='constructed';bandState.constructedAt=pgWorkerNow();}
    ...
    w.onmessage=event=>{
      ...
      if(data.type==='band-stage'){
        const idx=Number.isFinite(+data.bandIndex)?+data.bandIndex:i,s=coordinator.bandStates[idx]||bandState;
        s.stage=clean(data.stage||'stage-message',40);s.stageElapsedMs=Number.isFinite(+data.elapsedMs)?+data.elapsedMs:null;return;
      }
      ...
      if(data.type==='band-bootstrap-error'||data.type==='band-fatal-error'||data.type==='band-error'){
        const idx=Number.isFinite(+data.bandIndex)?+data.bandIndex:i,s=coordinator.bandStates[idx]||bandState;
        void fallback(null,{bandIndex:idx,stage:data.stage||s.stage,eventType:data.type,errorName:data.name,errorMessage:data.message});return;
      }
      if(data.type==='band-empty'){coordinator.remaining--;bandState.stage='band-empty';try{w.terminate();}catch(e){}finishIfReady();return;}
      if(data.type!=='band-complete')return;
      bandState.stage='band-complete';if(!coordinator.firstBandAt)coordinator.firstBandAt=pgWorkerNow();
      try{const rowOffset=(+data.startRow||0)*state.nx*4;coordinator.rgba.set(new Uint8ClampedArray(data.buffer),rowOffset);}
    ...
    w.onerror=event=>{
      ...
      void fallback(null,{bandIndex:i,stage:bandState.stage,eventType:'worker-error',errorName:event?.error?.name,errorMessage:event?.error?.message||event?.message||'Blob band worker failed',filename:event?.filename,line:event?.lineno,col:event?.colno});
    };
    w.onmessageerror=event=>{void fallback(null,{bandIndex:i,stage:bandState.stage,eventType:'message-error',errorName:'DataCloneError',errorMessage:event?.message||'child message could not be deserialized'});};
    try{w.postMessage({type:'build-band',jobId:msg.jobId,tier:msg.tier,nx:state.nx,ny:state.ny,settings:msg.settings,points:msg.points,band});bandState.postedAt=pgWorkerNow();}
  ...
  if(createError){const e=createError.error;await fallback(e,{bandIndex:createError.bandIndex,stage:createError.stage,eventType:'construct-error',errorName:e?.name,errorMessage:e?.message});return;}
  finishIfReady();

  async function fallback(error,meta={}){
    if(activeCoordinator!==coordinator||coordinator.fallingBack)return;
    coordinator.fallingBack=true;if(error){meta.errorName=meta.errorName||error?.name;meta.errorMessage=meta.errorMessage||error?.message||String(error);}
    const prepMs=coordinator.blobPrepElapsedMs,blobBytes=coordinator.blobSourceBytes;terminateCoordinator('band-failure');
    const diagnostic=await collectFailureDiagnostics(meta);buildWhole(msg,{fallbackReason:failureReason(diagnostic),blobPrepElapsedMs:prepMs,blobSourceBytes:blobBytes});
  }
  function finishIfReady(){
    ...
    const parallelMetrics={
      ...
      childWorkerKind:'blob-bundled-v136',parallelTierPolicy:'99-297-891',childCreateElapsedMs:coordinator.createElapsedMs||0,
      ...
      wallElapsedMs:+elapsed.toFixed(1),bandWorkerMinMs:+min(workerVals).toFixed(1),bandWorkerAvgMs:+((workerVals.length?sum(workerVals)/workerVals.length:0)).toFixed(1),
      bandWorkerMaxMs:+max(workerVals).toFixed(1),bandRasterizeTotalMs:+sum(rasterVals).toFixed(1),bandRasterizeMaxMs:+max(rasterVals).toFixed(1),
      bandColorTotalMs:+sum(colorVals).toFixed(1),bandSetupTotalMs:+sum(setupVals).toFixed(1),atomicFinalBuffer:true,
      partialFramesPublished:0,bandFramesPublished:0,sequentialBenchmarkRun:false
    };
    const buffer=coordinator.rgba.buffer;stopWorkers(coordinator);
    postMessage({type:'complete',jobId:msg.jobId,tier:+msg.tier,nx:state.nx,ny:state.ny,cells:coordinator.cells,context:msg.context||'regular',
      workerElapsedMs:+elapsed.toFixed(1),rasterizeElapsedMs:+max(rasterVals).toFixed(1),colorElapsedMs:+max(colorVals).toFixed(1),
      setupElapsedMs:+(coordinator.blobPrepElapsedMs+coordinator.createElapsedMs).toFixed(1),parallelMetrics,buffer},[buffer]);
  }
}
...
self.onmessage=event=>{
  ...
  if(PARALLEL_TIERS.has(tier)){
    ...
    try{buildWhole(msg,{fallbackReason:'non-progressive-tier'});}catch(error){postMessage({type:'error',jobId:msg.jobId,message:String(error&&error.message||error)});}
  }
};
