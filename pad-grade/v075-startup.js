...
(function installPadGrade075Startup(){
  ...
  function preapplyMapPrefs(){
    let prefs={};
    try{prefs=parse(localStorage.getItem(PREF_KEY),{})||{};}catch(e){prefs={};}
    const width=clamp(prefs.mapWidthPx,MIN_WIDTH,MAX_WIDTH);
    const height=clamp(prefs.mapHeightPx,MIN_HEIGHT,MAX_HEIGHT);
    const card=$('gpsMapCard'),wrap=document.querySelector('.gpsMapWrap');
    if(width&&card){
      card.style.width=`min(${width}px, calc(100vw - 24px))`;
  ...
  function primeLegacyState(project){
    ...
    const gps=project.gps||{};
    try{
      localStorage.setItem(MOBILE_KEY,JSON.stringify({
        settings:project.settings,
        readings:project.readings||{},
        readingMeta:project.readingMeta||{},
        gps:{reference:gps.reference||null,opposite:gps.opposite||null,targetIndex:Number.isInteger(gps.targetIndex)?gps.targetIndex:null},
  ...
  if(typeof legacyLoadLocal==='function'){
    window.loadLocal=function padGrade075LoadLocal(){
      ...
      if(project)primeLegacyState(project);
      legacyLoadLocal();
      if(project){
        try{
          ...
          measureMode=project.measureMode==='gps'?'gps':'manual';
          window.__padGradePreloadedProjectId=project.id;
          window.__padGradePreloadedMeasureMode=measureMode;
        }catch(e){console.warn('Pad Grade startup project prepaint failed',e);}
      }
    };
  }
  ...
  function cleanupRevealListeners(){
    ...
    if(attachedMap){
      try{if(mapIdleHandler)attachedMap.off('idle',mapIdleHandler);}catch(e){}
      try{if(mapActivityHandler){attachedMap.off('movestart',mapActivityHandler);attachedMap.off('zoomstart',mapActivityHandler);attachedMap.off('resize',mapActivityHandler);}}catch(e){}
    }
  ...
  function scheduleIdleReveal(){
    if(revealFinished||!sawIdle)return;
    if(revealTimer)clearTimeout(revealTimer);
    const now=Date.now();
    const minWait=Math.max(0,1000-(now-revealStartedAt));
    const quietWait=Math.max(0,220-(now-lastMapActivity));
    revealTimer=setTimeout(()=>{
      revealTimer=null;
      if(revealFinished||!sawIdle)return;
      if(Date.now()-lastMapActivity<210){scheduleIdleReveal();return;}
      finishReveal();
    },Math.max(minWait,quietWait));
  }

  function attachMapGate(map){
    if(revealFinished||!map)return;
    cleanupRevealListeners();
    attachedMap=map;lastMapActivity=Date.now();sawIdle=false;
    mapActivityHandler=()=>{
  ...
  function requestSettledReveal(){
    ...
    mapCreatedHandler=ev=>{
      const created=ev?.detail?.map||window.__padGradeMapInstance||null;
      if(created)attachMapGate(created);
    };
    window.addEventListener('padgrade-map-created',mapCreatedHandler);
  }
