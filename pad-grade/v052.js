...
(function installPadGradeV052(){
  ...
  function durableWrite(project){
    try{
      if(window.PadGradeNative&&typeof PadGradeNative.hasProjectFolder==='function'&&PadGradeNative.hasProjectFolder()&&typeof PadGradeNative.writeProjectFile==='function'){
        PadGradeNative.writeProjectFile(`${project.id}.padgrade`,JSON.stringify(project));
      }
    }catch(e){}
  }
  ...
  function renameProject(id){
    ...
    const project=readJson(projectKey(id),null);
    const index=readJson(INDEX_KEY,[]);
    if(!project||!Array.isArray(index))return false;
    const item=index.find(x=>x.id===id);
    if(!item)return false;

    const oldName=project.settings?.name||item.name||'Pad';
    const entered=prompt('Project name',oldName);
    ...
    item.status=(project.status||item.status)==='archived'?'archived':'open';
    project.status=item.status;

    writeJson(projectKey(id),project);
    writeJson(INDEX_KEY,index);
    durableWrite(project);

    if(localStorage.getItem(ACTIVE_KEY)===id){
      // Keep the live form in sync so v0.4.0 autosave cannot restore the old name.
      if($('projectName'))$('projectName').value=name;
      if($('v040ProjectName'))$('v040ProjectName').textContent=name;
  ...
  function installRenameInterceptor(){
    ...
    if(!dlg||dlg.dataset.v052Rename==='1')return;
    dlg.dataset.v052Rename='1';
    dlg.addEventListener('click',event=>{
      const button=event.target?.closest?.('button[data-act="rename"]');
      if(!button||!dlg.contains(button))return;
      const row=button.closest('[data-id]');
      if(!row)return;
      // Capture before v0.4.1's older handler so only one rename path runs.
      event.preventDefault();
      event.stopPropagation();
      event.stopImmediatePropagation();
      renameProject(row.dataset.id);
    },true);
  }

  function boot(){
    document.title='Pad Grade Mapper v0.5.2';
    installRenameInterceptor();
  }
