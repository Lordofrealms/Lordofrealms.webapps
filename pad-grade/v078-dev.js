...
(function installPadGrade079(){
  ...
  function finishActivation(){
    markActive();
    if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{setTimeout(markActive,60);setTimeout(markActive,500);},{once:true});
    else setTimeout(markActive,100);
  }
  ...
  function loadFreshSurface(){
    ...
    script.src=SURFACE_NEW;script.async=false;script.dataset.padgradeSurfaceV079='1';
    script.onload=finishActivation;
    script.onerror=()=>console.error('Pad Grade edge-locked surface module failed to load');
...
/* File-ID migration may enumerate the durable folder. Keep its local UI work off
 * the startup critical path when Android SAF indexing is still in progress. */
(function loadPadGrade080FileIds(){
  ...
  const load=()=>{
    ...
    const script=document.createElement('script');
    script.src='v080-file-id.js?v=20260828-1';script.async=false;script.dataset.padgradeV080FileId='1';
    script.onerror=()=>console.error('Pad Grade file-ID module failed to load');
    document.body.appendChild(script);
  };
  if(ready()){load();return;}
  timer=setInterval(()=>{if(ready()){clearInterval(timer);load();}},100);
  window.addEventListener('beforeunload',()=>{if(timer)clearInterval(timer);},{once:true});
})();

/* Read-only, temporary two-project elevation-change comparison. */
(function loadPadGrade081Comparison(){
  const loadBack=()=>{
    if(document.querySelector('script[data-padgrade-v087-compare-back]'))return;
    const script=document.createElement('script');
    script.src='v087-compare-back.js?v=20260829-1';script.async=false;script.dataset.padgradeV087CompareBack='1';
    document.body.appendChild(script);
  };
  const loadRenderOrder=()=>{
    const existing=document.querySelector('script[data-padgrade-v085-map-order]');
    if(existing){loadBack();return;}
    const render=document.createElement('script');
    render.src='v085-map-render-order.js?v=20260829-3';render.async=false;render.dataset.padgradeV085MapOrder='1';
    render.onload=loadBack;
    render.onerror=()=>{console.error('Pad Grade map render-order correction failed to load');loadBack();};
  ...
  const loadFix=()=>{
    ...
    fix.src='v083-project-compare-fix.js?v=20260829-1';fix.async=false;fix.dataset.padgradeV083CompareFix='1';
    fix.onload=loadRenderOrder;
    fix.onerror=()=>{console.error('Pad Grade comparison correction failed to load');loadRenderOrder();};
    document.body.appendChild(fix);
  };
  const loadUi=()=>{
    const existing=document.querySelector('script[data-padgrade-v081-compare]');
    if(existing){if(window.PadGradeProjectCompare)loadFix();else existing.addEventListener('load',loadFix,{once:true});return;}
    const ui=document.createElement('script');
    ui.src='v081-project-compare.js?v=20260829-1';ui.async=false;ui.dataset.padgradeV081Compare='1';
    ui.onload=loadFix;
    ui.onerror=()=>console.error('Pad Grade project comparison UI failed to load');
  ...
  core.src='project-compare-core.js?v=20260829-1';core.async=false;core.dataset.padgradeCompareCore='1';
  core.onload=loadUi;
  core.onerror=()=>console.error('Pad Grade project comparison core failed to load');
