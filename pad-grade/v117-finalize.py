...
runtime = WEB / 'v117-dev.js'
text = runtime.read_text(encoding='utf-8')
old = "  function requestCanonicalCommit(state,reason){\n    const candidate=chooseCandidate(state);"
new = "  function requestCanonicalCommit(state,reason){\n    const map=state?.map;if(!map)return false;\n    const candidate=chooseCandidate(state);"
if old in text:
    text = text.replace(old, new, 1)
elif new not in text:
    raise SystemExit('Could not locate requestCanonicalCommit insertion point')
runtime.write_text(text, encoding='utf-8')

selftest = WEB / 'v117-image-idle-selftest.js'
st = selftest.read_text(encoding='utf-8')
needle = "  'updateImage({image:frame.image,coordinates:coords})',\n"
addition = "  'const map=state?.map;if(!map)return false;',\n"
if addition not in st:
    if needle not in st:
        raise SystemExit('Could not locate self-test insertion point')
    st = st.replace(needle, needle + addition, 1)
selftest.write_text(st, encoding='utf-8')

web_entry = '''## v1.1.7 — development build
...
android_entry = '''## v1.1.7 — development build (89)
...
def prepend(path: Path, marker: str, entry: str):
...
prepend(WEB / 'CHANGELOG.md', '## v1.1.6 — development build', web_entry)
prepend(ANDROID / 'CHANGELOG.md', '## v1.1.6 — development build (88)', android_entry)
print('v1.1.7 finalization applied')
