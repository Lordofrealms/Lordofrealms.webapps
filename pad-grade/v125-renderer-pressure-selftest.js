...
assert(index.includes('<title>Pad Grade Mapper v1.2.5 DEV</title>'),'v1.2.5 title missing');
...
assert(gradle.includes('versionCode = 97'),'Android build 97 missing');
assert(gradle.includes('versionName = "1.2.5"'),'Android v1.2.5 missing');
assert(notes.includes('# Pad Grade Mapper v1.2.5 — DEV BUILD'),'v1.2.5 release notes heading missing');
...
// Exercise the dialog hold itself with a tiny DOM/runtime harness.
const raf=[];
let closeCount=0;
...
console.log('Pad Grade v1.2.5 renderer-pressure/project-boundary self-test passed');
