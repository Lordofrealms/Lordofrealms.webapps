/* Pad Grade v1.3.3 DEV — dedicated parallel final-891 coordinator.
 *
 * 99 and 297 remain single-worker calculations. 891 keeps one lifecycle-visible
 * coordinator worker and fans compute-only horizontal bands to a dedicated worker
 * script. Band workers never publish map/canvas frames. The coordinator assembles
 * one complete RGBA buffer offscreen and publishes exactly one final 891 result.
 */
'use strict';
...
const BAND_URL=new URL('heatmap-raster-band-worker-v133.js?v=20260901-1',self.location.href).href;
...
function buildWhole(msg,extra={}){
  ...
  postMessage({
    ...
    colorElapsedMs:colored.colorElapsedMs,setupElapsedMs:+(rasterStarted-state.started).toFixed(1),parallelMetrics:{
      ...
      fallbackReason:extra.fallbackReason||null,childWorkerKind:'dedicated-v133',
...
function startParallel(msg){
  ...
  const coordinator={
    jobId:msg.jobId,started:state.started,workers:[],remaining:desired,cancelled:false,
    rgba:new Uint8ClampedArray(state.nx*state.ny*4),cells:0,bandMetrics:[],
  ...
  for(let i=0;i<desired;i++){
    ...
    if(!band.rows){coordinator.remaining--;continue;}
    let w=null;
    try{w=new Worker(BAND_URL);}catch(e){createError=e;break;}
    ...
    w.onmessage=event=>{
      ...
      if(data.type==='band-error'){
        fallback(new Error(`band-${data.bandIndex??'?'}:${String(data.message||'band worker error')}`));
        return;
      }
    ...
    w.onerror=event=>{
      ...
      fallback(new Error(event?.message||'dedicated band worker failed'));
    };
    try{
      w.postMessage({
  ...
  if(createError){
    terminateCoordinator('child-construction-failed');
    buildWhole(msg,{fallbackReason:`child-construction-failed:${String(createError?.message||createError).slice(0,80)}`});
    return;
  }
  finishIfReady();
  ...
  function fallback(error){
    ...
    buildWhole(msg,{fallbackReason:`band-worker-failed:${String(error&&error.message||error).slice(0,120)}`});
  }
  function finishIfReady(){
    ...
    const parallelMetrics={
      ...
      childWorkerKind:'dedicated-v133',childCreateElapsedMs:coordinator.createElapsedMs||0,
    ...
    // This is the only successful parallel publication to the outer/main thread.
    // No child band is ever exposed to MapLibre or the canvas presenter.
    postMessage({
