...
const v5={
  ...
  settings:{width:64,length:76,cols:3,rows:3,target:64,tol:.5,refCorner:'SW',name:'Rollback Test'},
  ...
  measureMode:'gps',migration:{sourceVersion:5},dev:{diagnostics:false,custom:'preserve-me'}
};
...
const parsed=JSON.parse(text);assert.strictEqual(parsed.dev.custom,'preserve-me');
...
assert.deepStrictEqual(down.readings,v5.readings);assert.deepStrictEqual(down.readingMeta,v5.readingMeta);assert.deepStrictEqual(down.gps,v5.gps);assert.deepStrictEqual(down.settings,v5.settings);assert.deepStrictEqual(down.dev,v5.dev);
