/* Pad Grade v1.3.5 DEV — compute-only bundled heatmap band worker.
 *
 * The coordinator prepends surface-local-v078.js to this source and creates a
 * Blob worker. This worker performs compute only: it has no MapLibre/canvas
 * presentation authority and never publishes partial visible heatmap frames.
 */
'use strict';
...
stage('script-entered',{transport:'blob-bundled-v135'});
...
self.onmessage=event=>{
  ...
  try{buildBand(msg);}
    ...
    postMessage({type:'band-error',jobId:msg.jobId,bandIndex:+spec.index||0,stage:'build-band',...e});
  }
};
