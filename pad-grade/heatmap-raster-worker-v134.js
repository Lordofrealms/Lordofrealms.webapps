/* Pad Grade v1.3.4 DEV — dedicated parallel final-891 coordinator with failure diagnostics.
 *
 * 99 and 297 remain single-worker calculations. 891 still computes child bands
 * offscreen and publishes only one complete assembled RGBA buffer. v1.3.4 adds
 * staged child-worker diagnostics plus bounded nested-worker/asset probes only
 * after a band-worker failure.
 */
'use strict';
...
const BAND_URL=new URL('heatmap-raster-band-worker-v134.js?v=20260901-1',self.location.href).href;
...
function buildWhole(msg,extra={}){
  ...
  postMessage({
    ...
    colorElapsedMs:colored.colorElapsedMs,setupElapsedMs:+(rasterStarted-state.started).toFixed(1),parallelMetrics:{
      ...
      fallbackReason:extra.fallbackReason||null,childWorkerKind:'dedicated-v134',
...
function basename(value){
  try{return new URL(String(value||''),self.location.href).pathname.split('/').pop()||'';}catch(e){return '';}
}
...
async function assetProbe(url){
  ...
  let timer=null;
  try{
    if(ctrl)timer=setTimeout(()=>ctrl.abort(),900);
    const response=await fetch(url,{cache:'no-store',signal:ctrl?.signal});
    const text=await response.text();
    const type=String(response.headers?.get?.('content-type')||'').toLowerCase();
    return `${response.status}:${response.ok?'ok':'bad'}:${type.includes('javascript')?'js':type.includes('text')?'text':'other'}:${text.length}`;
  }catch(e){return `${clean(e?.name||'Error',24)}:${clean(e?.message||e,60)}`;}
  finally{if(timer)clearTimeout(timer);}
}
async function collectFailureDiagnostics(meta){
  const [blobResult,bandAsset,surfaceAsset]=await Promise.all([
    nestedBlobProbe(),
    assetProbe(BAND_URL),
    assetProbe(SURFACE_URL)
  ]);
  return {
    blobResult,bandAsset,surfaceAsset,
    bandIndex:Number.isFinite(+meta?.bandIndex)?+meta.bandIndex:-1,
    stage:clean(meta?.stage||'unknown',40),
    eventType:clean(meta?.eventType||'unknown',32),
    errorName:clean(meta?.errorName||'',32),
    errorMessage:clean(meta?.errorMessage||'',100),
    filename:basename(meta?.filename),
    line:Number.isFinite(+meta?.line)?+meta.line:0,
    col:Number.isFinite(+meta?.col)?+meta.col:0
  };
}
function failureReason(d){
  return [
    ...
    `band=${d.bandIndex}`,
    `stage=${d.stage}`,
    d.errorName?`name=${d.errorName}`:'',
    d.errorMessage?`msg=${d.errorMessage}`:'',
    d.filename?`file=${d.filename}`:'',
    d.line?`line=${d.line}`:'',
    d.col?`col=${d.col}`:'',
    `nestedBlob=${d.blobResult}`,
    `bandAsset=${d.bandAsset}`,
    `surfaceAsset=${d.surfaceAsset}`
...
function startParallel(msg){
  ...
  const coordinator={
    ...
    createStarted:pgWorkerNow(),firstBandAt:0
  };
  ...
  for(let i=0;i<desired;i++){
    ...
    w.onmessage=event=>{
      ...
      if(data.type==='band-bootstrap-error'||data.type==='band-fatal-error'||data.type==='band-error'){
        ...
        const s=coordinator.bandStates[idx]||bandState;
        fallback(null,{
          bandIndex:idx,stage:data.stage||s.stage,eventType:data.type,
          errorName:data.name,errorMessage:data.message
        });
        return;
      }
      if(data.type==='band-empty'){coordinator.remaining--;bandState.stage='band-empty';finishIfReady();return;}
      ...
      try{
        ...
        fallback(e,{bandIndex:i,stage:'merge-buffer',eventType:'merge-error',errorName:e?.name,errorMessage:e?.message});
        return;
      }
    ...
    w.onerror=event=>{
      ...
      fallback(null,{
        bandIndex:i,stage:bandState.stage,eventType:'worker-error',
        errorName:event?.error?.name,errorMessage:event?.error?.message||event?.message||'dedicated band worker failed',
        filename:event?.filename,line:event?.lineno,col:event?.colno
      });
    };
    w.onmessageerror=event=>{
      fallback(null,{
        bandIndex:i,stage:bandState.stage,eventType:'message-error',
        errorName:'DataCloneError',errorMessage:event?.message||'child message could not be deserialized'
      });
    };
    try{
      w.postMessage({
        type:'build-band',jobId:msg.jobId,tier:msg.tier,nx:state.nx,ny:state.ny,
        settings:msg.settings,points:msg.points,band
      });
      bandState.postedAt=pgWorkerNow();
    }catch(e){
      fallback(e,{bandIndex:i,stage:'post-message',eventType:'post-error',errorName:e?.name,errorMessage:e?.message});
      return;
    }
  }
  ...
  if(createError){
    ...
    const meta={bandIndex:createError.bandIndex,stage:createError.stage,eventType:'construct-error',errorName:e?.name,errorMessage:e?.message};
    fallback(e,meta);
    return;
  }
  finishIfReady();
  ...
  async function fallback(error,meta={}){
    ...
    if(error){
      meta.errorName=meta.errorName||error?.name;
      meta.errorMessage=meta.errorMessage||error?.message||String(error);
    }
    terminateCoordinator('band-failure');
    const diagnostic=await collectFailureDiagnostics(meta);
    buildWhole(msg,{fallbackReason:failureReason(diagnostic)});
  }
  function finishIfReady(){
    ...
    const parallelMetrics={
      ...
      childWorkerKind:'dedicated-v134',childCreateElapsedMs:coordinator.createElapsedMs||0,
      ...
      bandSetupTotalMs:+sum(setupVals).toFixed(1),atomicFinalBuffer:true,
      partialFramesPublished:0,bandFramesPublished:0
    };
    ...
    postMessage({
      ...
      context:msg.context||'regular',workerElapsedMs:+elapsed.toFixed(1),
      rasterizeElapsedMs:+max(rasterVals).toFixed(1),colorElapsedMs:+max(colorVals).toFixed(1),
      setupElapsedMs:+(coordinator.createElapsedMs||0).toFixed(1),parallelMetrics,buffer
    },[buffer]);
  }
}
...
self.onmessage=event=>{
  ...
  try{
    ...
    terminateCoordinator('exception');
    postMessage({type:'error',jobId:msg.jobId,message:String(error&&error.message||error)});
  }
};
