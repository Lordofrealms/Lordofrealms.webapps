/* Pad Grade v1.3.4 DEV — compute-only heatmap band worker with staged diagnostics.
 *
 * This worker still has no MapLibre/canvas/presentation authority. Diagnostic
 * messages report only bootstrap/compute stages and sanitized error metadata;
 * no partial heatmap frame is ever published outside the coordinator.
 */
'use strict';
...
function stage(name,extra={}){
  try{
    postMessage({type:'band-stage',stage:name,elapsedMs:+(pgWorkerNow()-pgWorkerStarted).toFixed(1),...extra});
  }catch(e){}
}

stage('script-entered',{workerLocationSameOrigin:true});
try{
  importScripts(SURFACE_URL);
  stage('surface-imported',{surfaceApiReady:!!self.PadGradeLocalSurface?.rasterize});
}catch(error){
  const e=safeError(error);
  try{postMessage({type:'band-bootstrap-error',stage:'surface-import',...e});}catch(ignore){}
  throw error;
}
stage('handler-ready');
...
function buildBand(msg){
  ...
  stage('build-received',{jobId:+msg.jobId||0,bandIndex:+spec.index||0,rows:+spec.rows||0});
  if(points.length<3){
  ...
  stage('raster-start',{jobId:+msg.jobId||0,bandIndex:+spec.index||0});
  ...
  stage('raster-complete',{jobId:+msg.jobId||0,bandIndex:+spec.index||0,cells:+surface.cells||0,elapsedMs:+(rasterEnded-rasterStarted).toFixed(1)});
  const colored=colorRaster(surface,target,tol,maxCut,maxFill),ended=pgWorkerNow();
  stage('color-complete',{jobId:+msg.jobId||0,bandIndex:+spec.index||0,elapsedMs:colored.colorElapsedMs});
...
self.onmessage=event=>{
  ...
  try{buildBand(msg);}
    ...
    postMessage({
      type:'band-error',jobId:msg.jobId,bandIndex:+spec.index||0,
      stage:'build-band',...e
    });
  }
};
