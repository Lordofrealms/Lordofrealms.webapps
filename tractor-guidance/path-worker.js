...
function makeDriveOrder(count,skipPass){
  const base=Array.from({length:count},(_,i)=>i);
  if(!skipPass)return base;
  const first=base.filter(i=>i%2===0);
  const second=base.filter(i=>i%2===1).reverse();
  return first.concat(second);
}
function scanlineSegments(poly,headingDeg,spacingM,startOrder,currentFix,jobId,coveragePriority='coverage',skipPass=false){
  ...
  if(range<=0||spacingM<=0){count=1;actual=0;startS=(minS+maxS)/2}
  else if(coveragePriority==='no-extra-overlap'){count=Math.max(1,Math.floor(range/spacingM)+1);actual=spacingM;const used=(count-1)*actual;startS=minS+(range-used)/2}
  else{count=Math.max(1,Math.ceil(range/spacingM)+1);actual=count===1?0:range/(count-1);startS=minS}

  let sVals=Array.from({length:count},(_,i)=>count===1?(minS+maxS)/2:startS+i*actual);
  if(startOrder==='high')sVals.reverse();
  if(startOrder==='near'&&currentFix&&sVals.length>1){const q=toXY([currentFix.lon,currentFix.lat],o),sCur=q.x*n.x+q.y*n.y;if(Math.abs(sCur-sVals.at(-1))<Math.abs(sCur-sVals[0]))sVals.reverse()}

  const rows=[];
  for(let row=0;row<sVals.length;row++){
    const s0=sVals[row];let xs=[];
    for(const rings of localPolys)for(const ps of rings)for(let i=0;i<ps.length-1;i++){const a=ps[i],b=ps[i+1];if((a.s<=s0&&b.s>s0)||(b.s<=s0&&a.s>s0))xs.push(a.t+(s0-a.s)*(b.t-a.t)/(b.s-a.s))}
    xs.sort((a,b)=>a-b);xs=xs.filter((v,i)=>i===0||Math.abs(v-xs[i-1])>0.01);
    const pairs=[];for(let i=0;i+1<xs.length;i+=2)if(Math.abs(xs[i+1]-xs[i])>=0.25)pairs.push([xs[i],xs[i+1]]);
    rows.push({row,s0,pairs});
  }

  const order=makeDriveOrder(rows.length,skipPass);
  const out=[];let lastPct=-1;
  for(let driveIdx=0;driveIdx<order.length;driveIdx++){
    const r=rows[order[driveIdx]];let pairs=r.pairs.slice();
    const reverse=driveIdx%2===1;
    if(reverse)pairs=pairs.reverse().map(([a,b])=>[b,a]);
    for(const [ta,tb] of pairs){
      const pa={x:u.x*ta+n.x*r.s0,y:u.y*ta+n.y*r.s0},pb={x:u.x*tb+n.x*r.s0,y:u.y*tb+n.y*r.s0};
      out.push({kind:skipPass?'skip-parallel':'parallel',pass:r.row+1,driveOrder:driveIdx+1,coords:[toLL(pa,o),toLL(pb,o)]});
    }
    const pct=Math.floor(46+45*(driveIdx+1)/Math.max(1,order.length));
    if(pct!==lastPct){progress(jobId,pct,skipPass?'Generating skip-pass route…':'Generating parallel passes…');lastPct=pct}
  }
  return{segments:out,fitInfo:{coveragePriority,count,actualSpacingM:actual,rangeM:range,skipPass}}
}
...
self.onmessage=e=>{
  ...
  try{
    ...
    const parallelLike=settings.pathType==='parallel'||settings.pathType==='skip-parallel';
    if(parallelLike){
      const skip=settings.pathType==='skip-parallel';
      progress(jobId,45,skip?'Generating skip-pass route…':'Generating parallel passes…');
      const result=scanlineSegments(safe,settings.parallelHeading,effWidth/FT_PER_M,settings.startOrder,currentFix,jobId,settings.coveragePriority||'coverage',skip);
    ...
    postMessage({type:'result',jobId,segments,plannedMiles,passGroups,activeAreaAcres,planMeta:{type:settings.pathType,heading:parallelLike?settings.parallelHeading:null,effectiveSpacingFt:effWidth,insetFt,coveragePriority:settings.coveragePriority||'coverage',actualSpacingFt:fitInfo?.actualSpacingM?fitInfo.actualSpacingM*FT_PER_M:null,skipPass:settings.pathType==='skip-parallel'}});
