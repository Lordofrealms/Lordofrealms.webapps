...
WEB_SECTION = '''## v1.1.2 — development build
...
ANDROID_SECTION = '''## v1.1.2 — development build
...
def insert_section(path_name: str, section: str) -> None:
    path = Path(path_name)
    text = path.read_text(encoding='utf-8')
    if '## v1.1.2 — development build' in text:
        return
    marker = '\n## v1.1.1 — development build\n'
    if marker not in text:
        raise RuntimeError(f'Could not find v1.1.1 insertion point in {path_name}')
    path.write_text(text.replace(marker, '\n' + section + '## v1.1.1 — development build\n', 1), encoding='utf-8')


insert_section('pad-grade/CHANGELOG.md', WEB_SECTION)
insert_section('pad-grade-android/CHANGELOG.md', ANDROID_SECTION)

normal_workflow = subprocess.check_output([
    'git', 'show',
    '25252ae684dac2be7851d284c05d9ed3bc98699a:.github/workflows/pad-grade-release-notes.yml'
], text=True)
Path('.github/workflows/pad-grade-release-notes.yml').write_text(normal_workflow, encoding='utf-8')

lines = Path('pad-grade/CHANGELOG.md').read_text(encoding='utf-8').splitlines()
start = lines.index('## v1.1.2 — development build')
...
Path('/tmp/pad-grade-v112-release-notes.md').write_text(intro + section, encoding='utf-8')
